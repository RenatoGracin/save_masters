...
matrix_data = scipy.io.loadmat("C:\\Users\\bujak\\Desktop\\FER\\5_godina\\DIPLOMSKI_PROJEKT\\DILPOMSKI_RAD\\plant_AE_classification\\clustering\\test_CDbw.mat") 

data = matrix_data['data']
all_labels = matrix_data['all_labels'][0]

for labels_ind,labels in enumerate(all_labels):
    labels = np.array(np.concatenate(labels.tolist()))
    valid_ind = np.where(labels>0)[0]
    # data = np.array([np.array([1,2,3]),np.array([4,5,6]),np.array([7,8,9]),np.array([4,5,6]),np.array([4,5,6]),np.array([4,5,6])])
    # labels = np.array([1,2,2,3,4,5])
    # CDbw_index = CDbw(data,labels-1,s=4)
    
    CDbw_index = CDbw(data[valid_ind,:],labels[valid_ind]-1,s=100)
    all_len = len(labels)
    data_len = len(labels[valid_ind])
    new_val = CDbw_index * data_len/all_len
    print("For labels " + str(labels_ind) + " index was: " + str(CDbw_index) + ", new_val: " + str(new_val))
