...
const Diskio_drvTypeDef  SD_Driver =	//Treba promijeniti u DMA -> Samo to? moguce, kod init. nema spomena o DMA!
{
  SD_initialize,
  SD_status,
  SD_read,
...
DRESULT SD_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
  DRESULT res = RES_ERROR;
	...
	if(count == 1 && BSP_SD_WriteBlocks((uint32_t*)buff,
																			(uint32_t)(sector),
																			count, SD_TIMEOUT) == MSD_OK) 
	{
		while(BSP_SD_GetCardState() != MSD_OK);
  ...
  else if(count != 1 && BSP_SD_WriteBlocks_DMA((uint32_t*)buff,
																							(uint32_t)(sector),
																							count) == MSD_OK)
	{
	res = RES_WAITDMA;
